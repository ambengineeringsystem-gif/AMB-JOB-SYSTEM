...
// No default columns; base category removed

function perBoardKey(b){ return 'kanban_categories_v1::' + (b || currentBoardId()); }
...
function saveLocalCategories(list, b){ try{ localStorage.setItem(perBoardKey(b), JSON.stringify(list)); }catch(e){} }
...
function renderModalList(list){
  ...
  // No base category section

  // All categories
  const otherLabel = document.createElement('h4');
  otherLabel.textContent = 'Categories';
  modalListEl.appendChild(otherLabel);

  cols.forEach((cat, idx) => {
...
function addCategory(name){
  ...
  saveLocalCategories(current, currentBoardId());
...
if(db){
  // Keep a per-board listener so remote categories are saved to the correct per-board local key.
  let categoriesUnsub = null;
  let listenedBoard = currentBoardId();
  function attachCategoriesListener(boardId){
    try{
      // detach previous
      if(typeof categoriesUnsub === 'function'){
        try{ categoriesUnsub(); }catch(e){}
        categoriesUnsub = null;
      }
      listenedBoard = boardId || currentBoardId();
      const catRef = ref(db, categoriesPathForBoard(listenedBoard));
      categoriesUnsub = onValue(catRef, snap => {
        const val = snap.val();
        if(!val) return;
        try{
          ...
          // save to the specific board's local key
          saveLocalCategories(list, listenedBoard);
    ...
    }catch(e){ console.warn('attachCategoriesListener failed', e); }
  }

  // attach initially for current board
  attachCategoriesListener(listenedBoard);

  // Reattach when board changes
  window.addEventListener('board-changed', (e)=>{
    try{ const bid = e?.detail?.boardId || currentBoardId(); attachCategoriesListener(bid); }catch(err){ console.warn('board-changed (categories) failed', err); }
  });
}
