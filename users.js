...
let visibleBoardsCheckboxContainer = null;
...
function renderVisibleCheckboxes(selected, columns){
  ...
  // If columns are provided in "Board / Category" format, group them by board
  const hasGrouped = cols.some(x => typeof x === 'string' && x.indexOf(' / ') !== -1);
  if(hasGrouped){
    // build map: boardTitle -> [categoryStrings]
    const map = {};
    cols.forEach(item => {
      try{
        ...
        if(parts.length >= 2){
          ...
          map[board].push({ full: item, label: cat });
        }else{
          if(!map['__ungrouped']) map['__ungrouped'] = [];
          map['__ungrouped'].push({ full: item, label: item });
        }
      }catch(e){ /* ignore malformed */ }
    });
    // render each group with a header
    Object.keys(map).forEach(board => {
      if(board !== '__ungrouped'){
        const hdr = document.createElement('div'); hdr.textContent = board; hdr.style.fontWeight = '600'; hdr.style.marginTop = '6px'; hdr.style.marginBottom = '4px'; hdr.style.fontSize = '13px'; visibleCheckboxContainer.appendChild(hdr);
      }
      const group = document.createElement('div'); group.style.display = 'flex'; group.style.flexWrap = 'wrap'; group.style.gap = '8px';
      map[board].forEach(entry => {
        const wrap = document.createElement('label');
        wrap.style.display = 'flex'; wrap.style.alignItems = 'center'; wrap.style.gap = '6px';
        const cb = document.createElement('input'); cb.type = 'checkbox'; cb.value = entry.full;
        cb.style.marginRight = '4px';
        if(Array.isArray(selected) && selected.includes(entry.full)) cb.checked = true;
        wrap.appendChild(cb);
        const txt = document.createElement('span'); txt.textContent = entry.label; txt.style.fontSize = '13px';
        wrap.appendChild(txt);
        group.appendChild(wrap);
      });
      visibleCheckboxContainer.appendChild(group);
    });
  } else {
...
// Render checkboxes for visible boards into visibleBoardsCheckboxContainer
function renderVisibleBoardsCheckboxes(selected, boards){
  if(!visibleBoardsCheckboxContainer) return;
  visibleBoardsCheckboxContainer.innerHTML = '';
  // get boards list
  let boardsList = Array.isArray(boards) && boards.length ? boards : [];
  if(boardsList.length === 0){
    // fallback to local boards
    try{
      const raw = localStorage.getItem('kanban_boards_v1');
      if(raw){
        const localBoards = JSON.parse(raw);
        boardsList = Object.keys(localBoards || {}).map(k => localBoards[k]);
      }
    }catch(e){}
  }
  if(boardsList.length === 0){
    ...
    visibleBoardsCheckboxContainer.appendChild(empty);
    return;
  }
  boardsList.forEach(b => {
    ...
    cb.style.marginRight = '4px';
    if(Array.isArray(selected) && selected.includes(b.id)) cb.checked = true;
    wrap.appendChild(cb);
    const txt = document.createElement('span'); txt.textContent = b.title || b.id; txt.style.fontSize = '13px';
    wrap.appendChild(txt);
    visibleBoardsCheckboxContainer.appendChild(wrap);
  });
}

function buildUsersModal(){
  // Support embedded rendering: if host page sets window._USERS_PANEL_CONTAINER to a DOM
  // element, render the users UI inside that container instead of a modal overlay.
  const embeddedContainer = (typeof window !== 'undefined' && window._USERS_PANEL_CONTAINER) ? window._USERS_PANEL_CONTAINER : null;
  // If we've already created the modal, ensure it's attached to the embedded container
  // (this handles the case where the host cleared the container when switching panels).
  if(usersModal){
    try{
      if(embeddedContainer){
        if(!embeddedContainer.contains(usersModal)){
          embeddedContainer.innerHTML = '';
          embeddedContainer.appendChild(usersModal);
        }
        // ensure visible
        usersModal.style.display = 'block';
      } else {
        if(!document.body.contains(usersModal)) document.body.appendChild(usersModal);
  ...
  if(!isEmbedded){
    ...
    // embedded mode: make wrapper transparent and let host control layout
    usersModal.style.display = 'block';
    usersModal.style.background = 'transparent';
    usersModal.style.position = 'relative';
  }
  ...
  if(!isEmbedded){
    ...
    // when embedded, make the panel adapt to the container width
  ...
  // visible columns
  // Visible columns - nicer layout: title + muted description + bordered checkbox area
  ...
  visibleWrap.style.marginBottom = '8px';

  const visibleTitle = document.createElement('div');
  visibleTitle.textContent = 'Visible columns';
  visibleTitle.style.fontSize = '13px';
  visibleTitle.style.fontWeight = '700';
  visibleWrap.appendChild(visibleTitle);

  const visibleDesc = document.createElement('div');
  visibleDesc.textContent = 'Check which columns this user may see.';
  visibleDesc.style.fontSize = '12px';
  visibleDesc.style.color = '#6b7280';
  visibleWrap.appendChild(visibleDesc);
  ...
  visibleCheckboxContainer.style.padding = '8px';
  visibleCheckboxContainer.style.border = '1px solid #eef3f8';
  visibleCheckboxContainer.style.borderRadius = '8px';
  visibleCheckboxContainer.style.background = '#fbfdff';

  // populate checkboxes
  renderVisibleCheckboxes(null);
  visibleWrap.appendChild(visibleCheckboxContainer);
  permWrap.appendChild(visibleWrap);

  // visible boards
  // Visible boards - nicer layout similar to columns
  const visibleBoardsWrap = document.createElement('div');
  visibleBoardsWrap.style.display = 'flex';
  visibleBoardsWrap.style.flexDirection = 'column';
  visibleBoardsWrap.style.gap = '6px';
  visibleBoardsWrap.style.marginBottom = '8px';

  const visibleBoardsTitle = document.createElement('div');
  visibleBoardsTitle.textContent = 'Visible boards';
  visibleBoardsTitle.style.fontSize = '13px';
  visibleBoardsTitle.style.fontWeight = '700';
  visibleBoardsWrap.appendChild(visibleBoardsTitle);

  const visibleBoardsDesc = document.createElement('div');
  visibleBoardsDesc.textContent = 'Check which boards this user may access.';
  visibleBoardsDesc.style.fontSize = '12px';
  visibleBoardsDesc.style.color = '#6b7280';
  visibleBoardsWrap.appendChild(visibleBoardsDesc);

  visibleBoardsCheckboxContainer = document.createElement('div');
  visibleBoardsCheckboxContainer.style.display = 'flex';
  visibleBoardsCheckboxContainer.style.flexWrap = 'wrap';
  visibleBoardsCheckboxContainer.style.gap = '8px';
  visibleBoardsCheckboxContainer.style.padding = '8px';
  visibleBoardsCheckboxContainer.style.border = '1px solid #eef3f8';
  visibleBoardsCheckboxContainer.style.borderRadius = '8px';
  visibleBoardsCheckboxContainer.style.background = '#fbfdff';
  // populate checkboxes
  renderVisibleBoardsCheckboxes(null);
  visibleBoardsWrap.appendChild(visibleBoardsCheckboxContainer);
  permWrap.appendChild(visibleBoardsWrap);
  ...
  let pass = (usersPassInput.value||'').trim();
    ...
    // collect checked visible boards
    let visibleBoards = null;
    try{
      const boxes = visibleBoardsCheckboxContainer.querySelectorAll('input[type=checkbox]');
      const sel = Array.from(boxes).filter(b=>b.checked).map(b=>b.value);
      visibleBoards = sel.length ? sel : null;
    }catch(e){ visibleBoards = null; }
  ...
  const payload = { username: name, password: pass, color, canCreate, canMove, canUpload, canViewAttachments, visibleColumns, visibleBoards };
    if(usersRef && db){
        ...
        onValue(ref(db, `/users/${name}`), snap => {
          ...
          const finalPayload = Object.assign({}, existing, payload);
              set(ref(db, `/users/${name}`), finalPayload).then(()=>{
                ...
                if(createCb) createCb.checked = false; if(moveCb) moveCb.checked = false; try{ const boxes = visibleCheckboxContainer.querySelectorAll('input[type=checkbox]'); boxes.forEach(b=>b.checked=false);}catch(e){} try{ const boxes2 = visibleBoardsCheckboxContainer.querySelectorAll('input[type=checkbox]'); boxes2.forEach(b=>b.checked=false);}catch(e){}
  ...
  close.addEventListener('click', ()=>{ usersModal.style.display = 'none'; try{ window.dispatchEvent(new CustomEvent('users-closed')); }catch(e){} });
  panel.appendChild(close);

  usersModal.appendChild(panel);
  if(isEmbedded){
    try{ embeddedContainer.innerHTML = ''; embeddedContainer.appendChild(usersModal); }catch(e){ document.body.appendChild(usersModal); }
  } else {
...
function renderUsers(list){
  ...
  if(u.canViewAttachments) parts.push('view-att'); else parts.push('no-view-att');
    ...
    if(Array.isArray(u.visibleBoards) && u.visibleBoards.length) parts.push('boards: ' + u.visibleBoards.join(','));
    ...
    edit.addEventListener('click', async ()=>{
      ...
      try{ createCb.checked = !!u.canCreate; moveCb.checked = !!u.canMove; }catch(e){}
      try{ uploadCb.checked = !!u.canUpload; viewAttachCb.checked = !!u.canViewAttachments; }catch(e){}
      // Attempt to load categories across all boards so the visible-columns list
      // shows every available category, then render checkboxes with the user's
      // current selections preserved.
      try{
        const combined = [];
        let boardsList = [];
        if(window._BOARDS && window._BOARDS.getAll){
          try{ const map = await window._BOARDS.getAll(); boardsList = Object.keys(map || {}).map(bk => map[bk]); }catch(e){ boardsList = []; }
        }
          ...
          try{
            await new Promise((resolve) => {
              onValue(ref(db, '/boards'), snap => {
                const val = snap.val() || {};
                boardsList = Object.keys(val || {}).map(bk => Object.assign({ id: bk }, (val[bk] || {})));
                resolve();
              }, { onlyOnce: true });
            });
          }catch(e){ /* ignore */ }
            ...
            await new Promise((resolve) => {
              try{
                onValue(ref(db, '/boards/' + id + '/categories'), snap => {
                  const val = snap.val() || {};
                  const items = Object.keys(val || {}).map(k => (val[k] && val[k].title) ? val[k].title : k);
                  items.forEach(c => combined.push(title + ' / ' + c));
                  resolve();
                }, { onlyOnce: true });
              }catch(e){ resolve(); }
            });
          }catch(e){ /* ignore per-board */ }
        }
        // Now render visible columns with user's selections
        const selected = Array.isArray(u.visibleColumns) ? u.visibleColumns : [];
        if(combined.length) renderVisibleCheckboxes(selected, combined); else renderVisibleCheckboxes(selected);
      }catch(e){ try{ renderVisibleCheckboxes(Array.isArray(u.visibleColumns) ? u.visibleColumns : []); }catch(err){} }
      // fetch and render boards for visibleBoards (existing behavior)
      try{
        if(window._BOARDS && window._BOARDS.getAll){
          window._BOARDS.getAll().then(boardsMap => {
            const boardsList = Object.keys(boardsMap || {}).map(k => boardsMap[k]);
            renderVisibleBoardsCheckboxes(Array.isArray(u.visibleBoards) ? u.visibleBoards : [], boardsList);
          }).catch(e => console.warn('Failed to load boards for editing user', e));
        }
      }catch(e){ console.warn('Failed to refresh boards for editing user', e); }
  ...
  if(isAdminAuthenticated){
    ...
    try{
      if(window._BOARDS && window._BOARDS.getAll){
        window._BOARDS.getAll().then(boardsMap => {
          const boardsList = Object.keys(boardsMap || {}).map(k => boardsMap[k]);
          renderVisibleBoardsCheckboxes(null, boardsList);
        }).catch(e => console.warn('Failed to load boards for user manager', e));
      }
    }catch(e){ console.warn('Failed to refresh boards for user manager', e); }
    // Additionally, attempt to load categories from all boards so the Visible
    // columns list includes every category across boards (not just current)
    (async function(){
      try{
        const combined = [];
        let boardsList = [];
        ...
        if(combined.length) renderVisibleCheckboxes(null, combined);
      }catch(e){ /* ignore overall */ }
    })();
    m.style.display = 'flex';
    try{ window.dispatchEvent(new CustomEvent('users-opened')); }catch(e){}
    ensureDefaultAdmin();
  }else{
...
window.addEventListener('admin-auth-success', (e)=>{
  ...
  if(u === 'admin'){
    // mark admin authenticated but do NOT auto-open the modal
    isAdminAuthenticated = true;
    try{ window._USERSAdminAuthenticated = true; }catch(e){}
    // prepare data for when the admin opens the Users manager
  ...
  // Also attempt to load categories across all boards so admins see every category
  (async function(){
    try{
      const combined = [];
      let boardsList = [];
  ...
  // do not auto-open the Users modal here; ensureDefaultAdmin still runs to provision admin user if needed
  try{ ensureDefaultAdmin(); }catch(e){}
  }else{
...
// When boards are updated elsewhere, refresh the visible-boards checkboxes
window.addEventListener('boards-updated', (e)=>{
  try{
    // preserve current selections
    const prev = [];
    if(visibleBoardsCheckboxContainer){
      const boxes = visibleBoardsCheckboxContainer.querySelectorAll('input[type=checkbox]');
      boxes.forEach(b => { if(b.checked) prev.push(b.value); });
    }
    // re-render with previous selections preserved when possible
    renderVisibleBoardsCheckboxes(prev, e?.detail?.boards);
  }catch(err){ console.warn('Failed to refresh visible boards after boards update', err); }
});
